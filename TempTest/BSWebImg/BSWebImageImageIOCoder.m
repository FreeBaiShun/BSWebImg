...
#import "BSWebImageCoderHelper.h"
#import "NSImage+BSWebCache.h"
#import <ImageIO/ImageIO.h>
#import "NSData+BSImageContentType.h"
...
@implementation BSWebImageImageIOCoder {
...
+ (instancetype)sharedCoder {
    static BSWebImageImageIOCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[BSWebImageImageIOCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
...
#if SD_UIKIT || SD_WATCH
            _orientation = [BSWebImageCoderHelper imageOrientationFromEXIFOrientation:orientationValue];
#endif
        }
    }
    ...
    if (_width + _height > 0) {
        ...
        if (partialImageRef) {
            ...
            CGColorSpaceRef colorSpace = BSCGColorSpaceGetDeviceRGB();
...
- (UIImage *)decompressedImageWithImage:(UIImage *)image
    ...
    if (optionsDict != nil) {
        ...
        if ([optionsDict[BSWebImageCoderScaleDownLargeImagesKey] isKindOfClass:[NSNumber class]]) {
            scaleDownLargeImagesOption = (NSNumber *)optionsDict[BSWebImageCoderScaleDownLargeImagesKey];
        }
...
#if SD_UIKIT || SD_WATCH
    NSInteger exifOrientation = [BSWebImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
    ...
    if (imageSource) {
        ...
        if (properties) {
            ...
            if (val) {
                ...
                result = [BSWebImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
...
+ (CGColorSpaceRef)colorSpaceForImageRef:(CGImageRef)imageRef {
    ...
    if (unsupportedColorSpace) {
        colorspaceRef = BSCGColorSpaceGetDeviceRGB();
    }
