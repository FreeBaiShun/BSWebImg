...
@implementation BSWebImageGIFCoder

+ (instancetype)sharedCoder {
    static BSWebImageGIFCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[BSWebImageGIFCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
...
#if SD_MAC
    BSAnimatedImageRep *imageRep = [[BSAnimatedImageRep alloc] initWithData:data];
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            BSWebImageFrame *frame = [BSWebImageFrame frameWithImage:image duration:duration];
        ...
        animatedImage = [BSWebImageCoderHelper animatedImageWithFrames:frames];
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format {
    ...
    NSArray<BSWebImageFrame *> *frames = [BSWebImageCoderHelper framesFromAnimatedImage:image];
