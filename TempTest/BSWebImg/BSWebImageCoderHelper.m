...
#import "UIImage+BSMultiFormat.h"
#import "NSImage+BSWebCache.h"
#import <ImageIO/ImageIO.h>
#import "BSAnimatedImageRep.h"

@implementation BSWebImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<BSWebImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(BSWebImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            BSWebImageFrame *frame = frames[i];
    ...
    CFRelease(imageDestination);
    BSAnimatedImageRep *imageRep = [[BSAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<BSWebImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    
    NSMutableArray<BSWebImageFrame *> *frames = [NSMutableArray array];
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            BSWebImageFrame *frame = [BSWebImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            BSWebImageFrame *frame = [BSWebImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
